_CONFIG = dict(
    ...
    MODEL=dict(
        NAME="HandNet",
        BACKBONE=dict(
            model_name="hiera_large_224",
            pretrain=True,
            out_feature_channel=1152,
            drop_path_rate=0.2,
        ),
        UV_HEAD=dict(
            in_channels=1152, 
    ...
    DATA=dict(
        ...
        FILES=[
            ...
            ("s3://fingerprint/hands6dof-data/keypoint/jsons/single_hand/assembly_hands_train.remove_interhand_iou0.01.filter30mm.json", 10.0, 2),            
    ...
    TRAIN=dict(
        DPLINK=dict(
            MINIBATCH_SIZE_PER_DIVICE=16,
            MINIBATCH_PER_EPOCH=512,
            NAME="train"
        ),
        WEIGHT_DECAY=1e-2,
        BASE_LR=1e-4,
    ...
    VAL=dict(
        BMKS=[
           ...
           dict(
                name="assembly_val",
                json_path = "s3://fingerprint/hands6dof-data/keypoint/jsons/single_hand/assembly_hands_val.remove_interhand_iou0.1.filter30mm.json",
                scale_enlarge=1.2,
            ),
        ],
        IMAGE_SHAPE=(224, 224),
